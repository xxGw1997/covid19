...
const initCharts = () => {
  ...
  store.item = citys[23].children as any;
  const data = citys.map((city) => {
    return {
      name: city.name,
      value: geoCoordMap[city.name].concat(city.total.nowConfirm),
...
const initPie = () => {
  ...
  const data = store.cityDetail.map((cd) => {
    return {
      name: cd.city,
      value: cd.nowConfirm,
    };
...
<template>
  <div class="box" :style="{ background: `url(${bg})` }">
    <div class="box-left" style="color:#ffffff">
    ...
    <div class="box-right" color-white>
      <table class="table" cellspacing="0" border="1">
        ...
        <transition-group
          ...
          tag="tbody"
        >
          <tr :key="item.name + item.total.confirm" v-for="item in store.item">
